...
('Tower of God', 'tower-of-god', 'Tower of God centers around a boy called Twenty-fifth Bam, who has spent most of his life trapped beneath a vast and mysterious Tower, with only his close friend, Rachel, to keep him company. When Rachel enters the Tower, Bam manages to open the door into it as well, and faces challenges at each floor of this tower as he tries to find his closest companion.', 'SIU', 'SIU', 'ongoing', 4.8, 8500000, ARRAY['Action', 'Adventure', 'Drama', 'Fantasy', 'Mystery', 'Supernatural', 'Webtoon'], ARRAY['Tower', 'Climbing', 'Powers', 'Friendship'], 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=600&fit=crop'),

('Solo Leveling', 'solo-leveling', 'In a world where hunters battle deadly monsters that emerge from gates connecting our world to other dimensions, Sung Jin-Woo is known as the weakest E-rank hunter. Despite his low rank, he continues to hunt monsters to pay for his mothers medical bills. During a seemingly routine dungeon raid, Jin-Woo and his party discover a hidden dungeon.', 'Chugong', 'DUBU (REDICE STUDIO)', 'completed', 4.9, 12000000, ARRAY['Action', 'Adventure', 'Fantasy', 'Supernatural', 'Webtoon'], ARRAY['Leveling', 'Monsters', 'Powers', 'Strong MC'], 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=600&fit=crop'),
...
-- Insert chapters for Tower of God
INSERT INTO chapters (manga_id, chapter_number, title, pages_count, published_at, content_url) 
SELECT 
  m.id,
  generate_series(1, 179) as chapter_number,
  CASE 
    WHEN generate_series(1, 179) = 1 THEN 'Headon'
    WHEN generate_series(1, 179) = 2 THEN 'Ball'
    WHEN generate_series(1, 179) = 179 THEN 'The Final Test'
    ELSE 'Chapter ' || generate_series(1, 179)
  END as title,
  floor(random() * 50 + 10)::integer as pages_count,
  NOW() - (179 - generate_series(1, 179)) * interval '1 week' as published_at,
  'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=800&h=1200&fit=crop' as content_url
FROM manga m WHERE m.slug = 'tower-of-god';
...
SELECT 
  ...
  generate_series(1, 179) as chapter_number,
  CASE 
    WHEN generate_series(1, 179) = 1 THEN 'The World''s Weakest Hunter'
    WHEN generate_series(1, 179) = 2 THEN 'The Double Dungeon'
    WHEN generate_series(1, 179) = 179 THEN 'The End of a Long Journey'
    ELSE 'Chapter ' || generate_series(1, 179)
  END as title,
  floor(random() * 30 + 15)::integer as pages_count,
  NOW() - (179 - generate_series(1, 179)) * interval '3 days' as published_at,
  'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=800&h=1200&fit=crop' as content_url
FROM manga m WHERE m.slug = 'solo-leveling';